...
export default function PreparadoPage() {
  ...
  const cargarDatos = useCallback(async () => {
    ...
    const [prep, st, pq, ub, cr, cat] = await Promise.all([
      ...
      supabase.from('stock_listo_planchar').select('id, docenas, catalogo_media_id, catalogo_media:catalogo_medias!catalogo_media_id(id, sku, codigo, talla, publico)').gt('docenas', 0),
      supabase.from('paquetes').select('id, codigo_paquete, docenas, total_pares, detalles_contenido, estado, preparador:usuarios!preparador_id(nombre), catalogo_media:catalogo_medias!catalogo_media_id(sku, codigo), ubicacion:ubicaciones!ubicacion_id(nombre)').order('created_at', { ascending: false }).limit(30),
      supabase.from('ubicaciones').select('id, nombre, tipo').eq('activo', true),
      supabase.from('cronograma_preparado').select('id, semana, anio, dia_semana, criterio, valor_criterio, preparador_id, preparador:usuarios!preparador_id(nombre)').eq('semana', semanaSeleccionada).eq('anio', anioSeleccionado),
